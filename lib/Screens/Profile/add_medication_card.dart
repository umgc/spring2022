...
class _UserProfileState extends State<AddMedicationCard> {
  ...
  _submit() {
    if (_formKey.currentState!.validate()) {
      ...
      Medication task = Medication(title: _title, dose: _dose);
      if (widget.medication == null) {
        task.status = 0;
        DatabaseHelper.instance.insertMedication(task);
      } else {
        // Update Task to Users Database
        task.id = widget.medication!.id;
        task.status = widget.medication!.status;
        DatabaseHelper.instance.updateMedication(task);
      }
